...
describe("PrizeGoal", () => {
  it("invites the family to pick a prize when nobody has one yet", () => {
    render(<PrizeGoal goals={[]} gemsByChild={{}} onSetGoal={vi.fn()} onClaim={vi.fn()} members={[]} />);
    expect(screen.getByText(/no prizes set yet/i)).toBeInTheDocument();
  });

  it("counts down the gems still to go", () => {
    render(<PrizeGoal goals={[goal()]} gemsByChild={{ Parker: 30 }} onSetGoal={vi.fn()} onClaim={vi.fn()} members={[]} />);
  ...
  it("says gem, not gems, when only one is left", () => {
    render(<PrizeGoal goals={[goal()]} gemsByChild={{ Parker: 49 }} onSetGoal={vi.fn()} onClaim={vi.fn()} members={[]} />);
  ...
  it("offers the prize once it's earned, rather than saying so forever", () => {
    render(<PrizeGoal goals={[goal()]} gemsByChild={{ Parker: 80 }} onSetGoal={vi.fn()} onClaim={vi.fn()} members={[]} />);
  ...
  it("starts a child with no gems yet at zero rather than blank", () => {
    render(<PrizeGoal goals={[goal({ memberId: "Isla", title: "Roller skates", gemCost: 40 })]} gemsByChild={{}} onSetGoal={vi.fn()} onClaim={vi.fn()} members={[]} />);
  ...
  it("saves a new prize and clears the form", async () => {
    ...
    render(<PrizeGoal goals={[]} gemsByChild={{}} onSetGoal={onSetGoal} onClaim={vi.fn()} members={[]} />);
